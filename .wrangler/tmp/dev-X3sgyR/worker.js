...
// .wrangler/tmp/bundle-65WSFG/checked-fetch.js
...
// .wrangler/tmp/bundle-65WSFG/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-65WSFG/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
var SITE_ORIGIN = "https://gradio.clothpath.com";
...
function countryFlagEmoji(countrycode) {
  ...
  return String.fromCodePoint(...[...cc].map((ch) => 127462 + ch.charCodeAt(0) - 65));
}
__name(countryFlagEmoji, "countryFlagEmoji");
function firstTags(tags, max) {
  return String(tags ?? "").split(",").map((tag) => tag.trim()).filter((tag) => tag.length > 0).slice(0, max);
}
__name(firstTags, "firstTags");
var PAGE_CSS = `
  :root {
    color-scheme: dark;
    --bg: #1a120b;
...
`;
function renderStationPage(station, related) {
  const uuid = String(station.stationuuid ?? "");
  const name = String(station.name ?? "").trim() || "Unnamed station";
  const country = String(station.country ?? "").trim() || String(station.countrycode ?? "").trim() || "Unknown";
  ...
  const description = "Listen to " + name + " live from " + country + " \u2014 free " + codec + " stream on Globe Radio. Spin the 3D globe to discover thousands more world radio stations.";
  ...
  const chips = [`<li class="chip">${escapeHtml(country)}</li>`];
  ...
  const relatedItems = related.filter((s) => s && typeof s === "object" && s.stationuuid && s.name).slice(0, 8).map((s) => {
    const metaBits = [];
    ...
    const meta = metaBits.length > 0 ? ` <span class="rel-meta">\xB7 ${escapeHtml(metaBits.join(" \xB7 "))}</span>` : "";
    return `<li><a href="/station/${encodeURIComponent(String(s.stationuuid))}">${escapeHtml(s.name)}${meta}</a></li>`;
  }).join("\n");
  const relatedSection = relatedItems.length > 0 ? `<section class="related">
...
<body>
  ...
  <main class="wrap">
    <p class="kicker">Live radio \xB7 streaming now</p>
    ...
    <a class="cta" href="${escapeHtml(globeHref)}">\u{1F4E1} Open on the 3D globe</a>
${relatedSection}
  </main>
  <footer>
    <div class="wrap">
      Powered by the <a href="https://www.radio-browser.info/">Radio Browser</a> community database \xB7 <a href="/">Globe Radio</a> \xB7 <a href="https://github.com/huanglizhuo/globe-radio">GitHub</a>
    </div>
  </footer>
...
__name(renderStationPage, "renderStationPage");
function renderStationNotFoundPage() {
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8">
...
<body>
  ...
  <main class="wrap">
    <p class="kicker">404 \xB7 signal lost</p>
    <h1>Station not found<span class="flag">\u{1F4FB}</span></h1>
    ...
    <a class="cta" href="/">\u{1F310} Spin the 3D globe instead</a>
  </main>
  <footer>
    <div class="wrap">
      Powered by the <a href="https://www.radio-browser.info/">Radio Browser</a> community database \xB7 <a href="/">Globe Radio</a> \xB7 <a href="https://github.com/huanglizhuo/globe-radio">GitHub</a>
...
__name(renderStationNotFoundPage, "renderStationNotFoundPage");
var DEBUG = false;
var UPSTREAM_API_BASE = "https://all.api.radio-browser.info";
var UPSTREAM_USER_AGENT = "GlobeRadio/1.0";
var UPSTREAM_CACHE_TTL_MS = 24 * 60 * 60 * 1e3;
var UPSTREAM_CACHE_RETENTION_S = 7 * 24 * 60 * 60;
var STATIONS_PER_SITEMAP = 5e3;
var STATION_UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
...
async function handleCountryDetection(request) {
  try {
    const countryCode = request.headers.get("CF-IPCountry");
    if (!countryCode) {
      console.log("\u{1F30D} No CF-IPCountry header found, using fallback");
    ...
    console.log(`\u{1F30D} Detected country: ${countryCode} from IP: ${request.headers.get("CF-Connecting-IP") || "unknown"}`);
    return new Response(JSON.stringify({
      country: countryCode,
      lat: null,
      // Will be handled by client-side mapping
      lon: null,
      // Will be handled by client-side mapping
      city: null,
      // Will be handled by client-side mapping
...
__name(handleCountryDetection, "handleCountryDetection");
function getEdgeCache() {
  return caches.default;
}
__name(getEdgeCache, "getEdgeCache");
async function fetchUpstreamJson(upstreamPathAndQuery) {
  try {
...
async function getCachedUpstreamJson(origin, keyPath, upstreamPathAndQuery, ctx) {
...
__name(getCachedUpstreamJson, "getCachedUpstreamJson");
function matchStationPageUuid(pathname) {
  const match = pathname.match(/^\/station\/([^/]+)\/?$/);
  if (!match) return null;
  return STATION_UUID_RE.test(match[1]) ? match[1] : null;
}
__name(matchStationPageUuid, "matchStationPageUuid");
...
__name(isPlayableStation, "isPlayableStation");
function stationNotFoundResponse() {
...
__name(stationNotFoundResponse, "stationNotFoundResponse");
async function handleStationPage(origin, uuid, ctx) {
  ...
  const station = Array.isArray(data) ? data[0] : void 0;
  ...
  if (/^[A-Z]{2}$/.test(countryCode)) {
    const countryData = await getCachedUpstreamJson(
      ...
      "/json/stations/bycountrycodeexact/" + countryCode + "?order=votes&reverse=true&hidebroken=true&limit=9",
      ctx
    );
    if (Array.isArray(countryData)) {
      related = countryData.filter(
        (s) => s && typeof s === "object" && s.stationuuid && s.stationuuid !== uuid
      ).slice(0, 8);
    }
  }
...
__name(handleStationPage, "handleStationPage");
...
async function getTopStations(origin, ctx) {
  const data = await getCachedUpstreamJson(
    origin,
    "/__cache/stationlist",
...
__name(getTopStations, "getTopStations");
function renderSitemapIndex(stationCount) {
...
__name(renderSitemapIndex, "renderSitemapIndex");
function renderPagesSitemap() {
  return '<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n  <url><loc>' + SITE_ORIGIN + "/</loc></url>\n</urlset>\n";
}
__name(renderPagesSitemap, "renderPagesSitemap");
function renderStationsSitemap(stations) {
  const urls2 = stations.map((station) => {
...
__name(renderStationsSitemap, "renderStationsSitemap");
async function handleStationsSitemapChunk(origin, ctx, chunk) {
...
__name(handleStationsSitemapChunk, "handleStationsSitemapChunk");
var worker_default = {
  async fetch(request, _env, ctx) {
    ...
    try {
      ...
      const page = await (0, import_kv_asset_handler.getAssetFromKV)(
        {
          request,
      ...
      if (!DEBUG) {
        try {
          let notFoundResponse = await (0, import_kv_asset_handler.getAssetFromKV)(
            {
              request,
              waitUntil: ctx.waitUntil.bind(ctx)
            },
